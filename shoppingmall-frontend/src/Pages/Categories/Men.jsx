...
const Men = () => {
  const navigate = useNavigate();
    ...
    const getAllProducts = async () => {
        try {
            setLoading(true);
            const { data } = await axios.get(`http://localhost:8080/product/allproducts/mens`, { withCredentials: false });
            setLoading(false);
            // console.log(data);
            setProducts(data);
  ...
  return (
    <Layout title={"ShoppingMall- Shop Now"}>
            ...
            <div className="col-md-12 order-md-2 order-1 pt-2">
                <h1 className="text-center">Men's Category</h1>
                <hr />
                        ...
                        return (
                            <>
                                <div className="card m-4" style={{ width: '18rem' }} >
                                    <img src={`http://localhost:8080/product/${p.id}/image`} className="card-img-top"
                                     onClick={() => showModal(p.id)} alt={p.name} height={"300px"} />
